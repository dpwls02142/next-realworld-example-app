...
const ArticleAPI = {
  all: (page, limit = 10) => axios.get(`${article}?${getQuery(limit, page)}`),
  ...
  favorite: (slug) => axios.post(`${withSlug(slug)}/favorite`),
  ...
  get: (slug) => axios.get(withSlug(slug)),

  unfavorite: (slug) => axios.delete(`${withSlug(slug)}/favorite`),
